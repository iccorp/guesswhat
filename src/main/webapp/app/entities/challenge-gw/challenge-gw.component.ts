...
@Component({
    selector: 'jhi-challenge-gw',
    templateUrl: './challenge-gw.component.html'
})
export class ChallengeGwComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/challenge-gw'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/challenge-gw', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ChallengeGw) {
        return item.id;
    }
